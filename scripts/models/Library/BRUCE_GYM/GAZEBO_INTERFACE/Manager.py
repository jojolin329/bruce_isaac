...
class GazeboInterface(object):
    ...
    def close(self):
        try:
            ...
            pass
        # Optionally: add shared memory cleanup
        for seg in [self.WORLD_PARAMETER, self.MODEL_PARAMETER, self.JOINT_STATE,
                    self.JOINT_TORQUE_COMMAND, self.POSITION_PID_GAIN,
                    self.JOINT_POSITION_COMMAND, self.JOINT_LIMIT, self.TORQUE_LIMIT,
                    self.BODY_POSE, self.IMU_STATE, self.LIMB_CONTACT,
                    self.BODY_FORCE, self.BODY_TORQUE]:
            try:
                seg.disconnect_segment()  # Or seg.cleanup(), if implemented
            except:
                pass
